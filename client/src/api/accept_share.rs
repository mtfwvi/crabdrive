...
use crabdrive_common::payloads::node::response::share::{GetShareInfoResponse, PostAcceptShareResponse};
...
fn parse_share_url(url: &str) -> Result<(NodeId, RawEncryptionKey)> {
    ...
    let re = Regex::new(r"([^/]+)/?$")?;
    let Some(caps) = re.captures(url) else {return Err(anyhow!("could not parse URL"))};
    let url_end = &caps[1];
    ...
    let Some(share_id) = share_id else { return Err(anyhow!("could not parse share id")) };
...
pub async fn accept_share(url: &str) -> Result<()> {
    ...
    let unwrapped_key = unwrap_key(&share_info.wrapped_metadata_key, &wrapping_encryption_key).await?;
