...
pub async fn get_single_node(node_id: NodeId) -> Result<DecryptedNode, String> {
    let get_node_result = get_node(node_id, &"".to_string()).await;
    if let Err(js_error) = get_node_result {
        return Err(format!(
            "could not get node with id {}: {:?}",
            node_id, js_error
        ));
    }
    ...
    match get_node_response {
        ...
        GetNodeResponse::NotFound => Err("get_node returned 404".to_string()),
    }
}
