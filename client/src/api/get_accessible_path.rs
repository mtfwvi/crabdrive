...
use crabdrive_common::payloads::node::response::node::{GetAccessiblePathResponse, GetNodeResponse};
...
pub async fn get_accessible_path(node_id: NodeId) -> Result<Vec<DecryptedNode>> {
    ...
    let encrypted_path = match response {
        GetAccessiblePathResponse::Ok(encrypted_path) => {
            encrypted_path
        }
        GetAccessiblePathResponse::NotFound => {
            return Err(anyhow!("Server returned 404 when getting path to shared node"));
        }
    };

    let first_node_id = encrypted_path.get(0).ok_or(anyhow!("path returned by server is empty"))?;
