...
use crabdrive_common::payloads::node::{request::share::*, response::node::*, response::share::*};
...
pub async fn test_get_accept_share_info() {
    ...
    let info_res = user_b
        .get(routes::node::share::get_share_accept_info(share_id))
        .await;
...
pub async fn test_get_invalid_accept_share_info() {
    ...
    let info_res = user
        .get(routes::node::share::get_share_accept_info(UUID::random()))
        .await;
...
pub async fn test_accept_invalid_share() {
    ...
    let accept_res = user
        .post(routes::node::share::accept_share(UUID::random()))
        .json(&accept_payload)
        .await;
...
pub async fn test_accessible_path_shared() {
    ...
    let request = user_b
        .get(routes::node::accessible_path(child_folder.id))
        .await;
...
// Info about accepted node share

#[tokio::test]
pub async fn test_get_node_share_info() {
    let ctx = TestContext::new(2).await;
    let user_a = ctx.get_user(0);
    let user_b = ctx.get_user(1);
    ...
    let info_res = user_b
        .get(routes::node::share::get_node_share_info(folder.id))
        .await;

    assert_eq!(info_res.status_code(), StatusCode::OK);

    let GetNodeShareInfo::Ok(info) = info_res.json() else {
        panic!("Expected Ok");
    };

    assert_eq!(info.node_id, folder.id);
    assert_eq!(info.wrapped_metadata_key, user_b.keys.master_key);
}

#[tokio::test]
pub async fn test_get_node_share_info_not_found() {
    let ctx = TestContext::new(1).await;
    let user_a = ctx.get_user(0);
    let folder = user_a.generate_random_folder().await;
